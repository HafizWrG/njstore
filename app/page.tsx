...
import {
    ...
    Home, User, X, Zap, Settings, ToggleLeft, ToggleRight, Wallet, MinusCircle, PlusCircle, History, Receipt, Lock, ExternalLink, TrendingUp,
    Smartphone, Download, Star, Upload, Info, LayoutGrid, HardDrive, Hash, Share2, Youtube, Instagram, ChevronLeft, ChevronRight
} from 'lucide-react';
...
// --- COMPONENTS ---
import { Background, KodesetInput, FileUploadField, MultipleFileUploadField, KodesetProductCard, Toast } from '../components/UI';
import { StorefrontView } from '../components/views/StorefrontView';
import { AppStoreView } from '../components/views/AppStoreView';
import { getPlatformIcon, formatDownloads, formatExternalUrl } from '../utils/helpers';
...
export default function WuregStore() {
    ...
    // App Buy Options
    const [appBuyModalOpen, setAppBuyModalOpen] = useState(false);
    const [appRedeemCode, setAppRedeemCode] = useState('');
    ...
    // CRUD & Modals
    const [modalType, setModalType] = useState<'product' | 'app' | 'payment' | 'voucher' | 'contact' | 'invoice' | 'expense_detail' | 'topup_balance' | 'redeem_code' | null>(null);
    ...
    const refreshAdminData = async () => {
        ...
        const [t, p, a, v, pmAll, cmAll, exp, rc] = await Promise.all([
        ...
        if (rc.data) setRedeemCodes(rc.data);
    };
    ...
    const handleRedeemApp = async () => {
        if (!appRedeemCode) return showToast("Masukkan kode redeem", "error");
        setIsRedeeming(true);
        try {
            const { data, error } = await supabase.from('redeem_codes')
                .select('*')
                .eq('code', appRedeemCode)
                .eq('is_used', false)
                .single();

            if (error || !data) {
                showToast("Kode redeem tidak valid atau sudah digunakan", "error");
            } else {
                await supabase.from('redeem_codes').update({ is_used: true }).eq('id', data.id);
                showToast("Redeem Berhasil! Memulai unduhan...", "success");
                setAppBuyModalOpen(false);
                setAppRedeemCode('');
    ...
    const handleCheckout = async (platform: 'wa' | 'ig') => {
        ...
        if (!error) {
            ...
            if (platform === 'wa') {
                ...
                const ig = contactMethods.find(c => c.platform_name.toLowerCase().includes('ig') || c.platform_name.toLowerCase().includes('instagram'))?.url || `https://www.instagram.com/hfz.wrg/`;
                const parsedIg = formatExternalUrl(ig);
                window.open(parsedIg, '_blank');
            }
    ...
    return (
        <div className="min-h-screen font-sans text-zinc-800 pb-24 md:pb-0 selection:bg-indigo-100 selection:text-indigo-600">
                                    ...
                                    <div className="space-y-8">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                <h3 className="font-bold text-lg text-zinc-900">Apps & Games</h3>
                                            </div>
                                        ...
                                        <div className="space-y-4 border-t border-zinc-200 pt-8">
                                            <div className="flex items-center justify-between">
                                                <h3 className="font-bold text-lg text-zinc-900">Redeem Codes</h3>
                                                <span className="text-xs text-zinc-500 bg-zinc-100 px-2 py-1 rounded">Untuk WDA Apps berbayar</span>
                                            </div>
                                            <button onClick={() => { setEditingItem(null); setFormData({ is_used: false }); setModalType('redeem_code'); }} className="w-full py-3 border-2 border-dashed border-zinc-300 rounded-xl text-zinc-400 font-bold hover:border-indigo-500 hover:text-indigo-500 transition-colors bg-zinc-50">+ Buat Custom Redeem Code</button>
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                {redeemCodes.map(rc => (
                                                    <div key={rc.id} className="flex justify-between items-center p-4 bg-zinc-50 rounded-xl border border-zinc-200">
                                                        <div>
                                                            <div className="font-bold text-sm text-zinc-900">{rc.code}</div>
                                                            <div className={`text-[10px] font-bold mt-1 uppercase tracking-wider ${rc.is_used ? 'text-red-500' : 'text-green-500'}`}>
                                                                {rc.is_used ? 'Sudah Terpakai' : 'Tersedia'}
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                            <button onClick={() => { setEditingItem(rc); setFormData(rc); setModalType('redeem_code'); }} className="p-1.5 bg-white border border-zinc-200 rounded-lg text-zinc-600"><Edit3 size={16} /></button>
                                                            <button onClick={() => handleDelete('redeem_codes', rc.id)} className="p-1.5 bg-white border border-red-200 rounded-lg text-red-500"><Trash2 size={16} /></button>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
            ...
            {selectedApp && (
                <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-md p-0 md:p-6 animate-in fade-in">
                    <div className="bg-white w-full md:max-w-2xl rounded-t-[32px] md:rounded-[32px] overflow-hidden shadow-2xl max-h-[90vh] md:max-h-[85vh] flex flex-col animate-in slide-in-from-bottom-10 relative">
                        ...
                        {/* Sticky Close Button (Dipindah ke luar agar tidak terpotong) */}
            ...
            {/* APP BUY OPTIONS MODAL */}
            {appBuyModalOpen && selectedApp && (
                <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in zoom-in-95">
                    <div className="bg-white w-full max-w-sm rounded-[32px] p-6 shadow-2xl relative">
                        <button onClick={() => setAppBuyModalOpen(false)} className="absolute top-4 right-4 p-2 bg-zinc-100 rounded-full text-zinc-500 hover:bg-zinc-200"><X size={18} /></button>
                        <h3 className="font-bold text-xl text-zinc-900 mb-2">Dapatkan Aplikasi</h3>
                        <p className="text-zinc-500 text-xs mb-6">Pilih metode untuk mendapatkan aplikasi ini.</p>

                        <div className="space-y-4">
                            <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-200">
                                <label className="text-xs font-bold text-zinc-500 mb-2 block">Punya Kode Redeem?</label>
                                <div className="flex gap-2">
                                    <input
                                        type="text"
                                        placeholder="Masukkan Kode"
                                        value={appRedeemCode}
                                        onChange={e => setAppRedeemCode(e.target.value.toUpperCase())}
                                        className="flex-1 w-full bg-white border border-zinc-200 rounded-xl px-3 py-3 text-sm outline-none focus:border-blue-500 uppercase font-bold"
                                    />
                                    <button disabled={isRedeeming} onClick={handleRedeemApp} className="px-5 bg-zinc-900 text-white text-xs font-bold rounded-xl hover:bg-zinc-800 transition-colors">
                                        {isRedeeming ? '...' : 'REDEEM'}
                                    </button>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20"
                            >
                                Bayar Rp {selectedApp.price.toLocaleString()}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {selectedProduct && (
                <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/40 backdrop-blur-md p-0 md:p-4 animate-in fade-in">
                    <div className="bg-white w-full md:max-w-md rounded-t-[32px] md:rounded-[32px] p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-10">
                        ...
                        {checkoutStep === 1 ? (
                            <div className="space-y-4">
                                <KodesetInput placeholder="Email / WhatsApp" value={buyerForm.email} onChange={(e: any) => setBuyerForm({ ...buyerForm, email: e.target.value, name: 'Customer' })} />
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 pt-2">
                                    <button onClick={() => setCheckoutStep(1)} className="w-full sm:flex-1 py-4 bg-white border border-zinc-200 font-bold rounded-[20px] text-zinc-600 hover:bg-zinc-50">Kembali</button>
                                    <div className="flex w-full sm:flex-[2] gap-2">
                                        <button disabled={isSubmitting} onClick={() => handleCheckout('wa')} className="flex-1 py-4 bg-green-500 text-white font-bold rounded-[20px] shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
                                            <MessageCircle size={18} /> WhatsApp
                                        </button>
                                        <button disabled={isSubmitting} onClick={() => handleCheckout('ig')} className="flex-1 py-4 bg-gradient-to-tr from-pink-500 to-purple-500 text-white font-bold rounded-[20px] shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
                        ...
                        <div className="space-y-4 mb-6">
                            ...
                            {modalType === 'app' && (
                                <>
                                    ...
                                    <FileUploadField label="Icon URL / Upload" value={formData.icon_url} onChange={(val: string) => setFormData((prev: any) => ({ ...prev, icon_url: val }))} onUpload={(e: any) => handleFileUpload(e, 'store_assets', 'icon_url')} isUploading={isUploading} />
                                    <FileUploadField label="Banner URL / Upload" value={formData.banner_url} onChange={(val: string) => setFormData((prev: any) => ({ ...prev, banner_url: val }))} onUpload={(e: any) => handleFileUpload(e, 'store_assets', 'banner_url')} isUploading={isUploading} />
                                    <FileUploadField label="APK/Store Link / Upload File" value={formData.apk_url} onChange={(val: string) => setFormData((prev: any) => ({ ...prev, apk_url: val }))} onUpload={(e: any) => handleFileUpload(e, 'store_assets', 'apk_url')} isUploading={isUploading} accept="*" />
                            ...
                            {modalType === 'redeem_code' && (
                                <>
                                    <KodesetInput placeholder="Kode Redeem (ex: WDA2024)" value={formData.code || ''} onChange={(e: any) => setFormData({ ...formData, code: e.target.value.toUpperCase() })} />
                                    <div className="flex items-center gap-2 mt-2 px-2 mb-4">
                                        <span className="text-sm font-bold text-zinc-500">Sudah Terpakai (is_used)?</span>
                                        <input type="checkbox" className="w-5 h-5" checked={formData.is_used ?? false} onChange={(e) => setFormData({ ...formData, is_used: e.target.checked })} />
                                    </div>
                                    <button onClick={() => handleSaveItem('redeem_codes', formData)} className="w-full py-4 bg-indigo-600 text-white font-bold rounded-[20px] shadow-lg mt-2">Simpan Kode</button>
                                </>
                            )}
                        </div>
            ...
            {isContactModalOpen && (
                <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-md rounded-[32px] p-6 shadow-2xl relative overflow-hidden">
                        <button onClick={() => setIsContactModalOpen(false)} className="absolute top-4 right-4 p-2 bg-zinc-100 rounded-full text-zinc-500 hover:bg-zinc-200 z-10"><X size={18} /></button>
                        <div className="text-center mb-6 mt-2 relative z-10">
                            <h3 className="font-black text-2xl text-zinc-900 tracking-tight">Hubungi Kami</h3>
                        </div>

                        <div className="relative group">
                            {/* Left/Right scroll hints */}
                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-8 h-full bg-gradient-to-r from-white to-transparent pointer-events-none z-10"></div>
                            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-full bg-gradient-to-l from-white to-transparent pointer-events-none z-10"></div>

                            <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 pt-2 px-2 scrollbar-hide">
                                {contactMethods.map((c, i) => {
                                    const isWa = c.platform_name.toLowerCase().includes('wa') || c.platform_name.toLowerCase().includes('whatsapp');
                                    const isIg = c.platform_name.toLowerCase().includes('ig') || c.platform_name.toLowerCase().includes('insta');

                                    return (
                                        <a key={c.id} href={formatExternalUrl(c.url)} target="_blank" rel="noreferrer" className="shrink-0 w-[160px] snap-center flex flex-col items-center gap-3 p-6 bg-white border border-zinc-100 rounded-[24px] hover:border-indigo-200 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 group shadow-sm text-center">
                                            <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-md transition-transform group-hover:scale-110 ${isWa ? 'bg-green-50 text-green-500' : isIg ? 'bg-pink-50 text-pink-500' : 'bg-blue-50 text-blue-500'}`}>
                                                {isWa ? <MessageCircle size={28} /> : isIg ? <Instagram size={28} /> : <ExternalLink size={28} />}
                                            </div>
                                            <div>
                                                <div className="font-bold text-zinc-900 leading-tight">{c.platform_name}</div>
                                                <div className="text-[10px] text-zinc-400 mt-1 uppercase font-bold tracking-wider">Tap To Open</div>
                                            </div>
                                        </a>
                                    );
                                })}
                                ...
                                {contactMethods.length === 0 && (
                                    <div className="w-full text-center py-8 text-zinc-400 text-sm font-medium">Belum ada kontak tersedia.</div>
                                )}
                            </div>
                        </div>
                    </div>
